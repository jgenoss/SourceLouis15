...
void ItemPrice::Init()
{
	...
	SetCompleteHook(0xFF, 0x005865A4, &this->ShowBuyPrice);
	SetCompleteHook(0xFF, 0x005866D6, &this->ShowSellPrice);

	SetCompleteHook(0xE8,0x0586534,&this->SetPriceRecv);
	SetCompleteHook(0xE8,0x05865BB,&this->SetPriceRecv);

	SetCompleteHook(0xE9,0x005866EC,&ChangeValueColor);

}

__declspec(naked) void ChangeValueColor()
{
	static DWORD AdressFinal = 0x005866F4;
	_asm
	{
...
void ItemPrice::SetInfo(int index, int level,int newopt, int type, int value, int buysell, int sellvalue)
{
	...
	for(int n=0;n < MAX_ITEMPRICE;n++)
	{
		...
		if (this->m_ItemPrice[n].ItemIndex == -1)
		{
			...
			//sprintf(log,"%d %d %d %d",index,level,newopt,value);
